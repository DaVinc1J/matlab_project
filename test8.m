function test7()
	...
	function move_snake()
		...
		if ~can_move_to(ui, user, new_head)
			...
			save_score_to_leaderboard(user.snake.score);
			return;
		end
	...
	function update_map()
		...
		for i = 1:size(ui.updated_positions, 1)
			...
			idx = sub2ind(size(ui.map), pos(1), pos(2));
			set(ui.text(idx), 'String', ui.map(pos(1), pos(2)));
			if isKey(color_map, ui.map(pos(1), pos(2)))
				set(ui.text(idx), 'Color', color_map(ui.map(pos(1), pos(2))));
			end
		end
		ui.updated_positions = [];
	end

	function save_score_to_leaderboard(score)
...
endunction test8()
	...
	function move_snake()
		...
		body_indices = sub2ind(size(ui.map), user.snake.body(:,1), user.snake.body(:,2));
		ui.map(body_indices) = char.player.body;
		ui.map(user.snake.body(1,1), user.snake.body(1,2)) = char.player.(['head_' user.snake.direction]);
		ui.updated_positions = [ui.updated_positions; user.snake.body];
		user.update_map = true;
	end
	...
	function tf = can_move_to(ui, user, new_head)
		...
		body_to_check = user.snake.body(1:end-1,:);
		current_cell = ui.map(new_head(1), new_head(2));

		if ismember(current_cell, char.impassable.wall) || ...
			ismember(new_head, body_to_check, 'rows')
			tf = false;
		else
			tf = true;
		end
	...
	function update_map()
		color_map = containers.Map(...
		{char.player.head_up, char.player.body, char.player.food, ...
		char.impassable.wall, char.passable.floor}, ...
		{colours.player_head, colours.player_body, colours.food, colours.wall, colours.floor});
...
function ui = create_graphics(char, font, ui, colours)
	...
	for r = 1:ui.rows
		for c = 1:ui.cols
			current_char = ui.map(r,c);
			if ismember(current_char, {char.impassable.top_left, char.impassable.top_right, ...
				char.impassable.bottom_left, char.impassable.bottom_right, ...
				char.impassable.top_bottom, char.impassable.left_right})
				cell_colour = colours.wall;
			elseif current_char == char.passable.floor
				cell_colour = colours.floor;
			elseif current_char == char.player.body
...
function [ui, user] = create_map(char, ui, user)
	...
	for r = 1:ui.rows
		for c = 1:ui.cols
			if r == 1 && c == 1
				ui.map(r,c) = char.impassable.top_left;
			elseif r == 1 && c == ui.cols
				ui.map(r,c) = char.impassable.top_right;
			elseif r == ui.rows && c == 1
				ui.map(r,c) = char.impassable.bottom_left;
			elseif r == ui.rows && c == ui.cols
				ui.map(r,c) = char.impassable.bottom_right;
			elseif r == 1 || r == ui.rows
				ui.map(r,c) = char.impassable.top_bottom;
			elseif c == 1 || c == ui.cols
				ui.map(r,c) = char.impassable.left_right;
			else
				ui.map(r,c) = char.passable.floor;
			end
		end
	end

	user.player_pos = [ceil(ui.rows/2), ceil(ui.cols/2)];
	ui.map(user.player_pos(1), user.player_pos(2)) = char.player.head_right;
end
